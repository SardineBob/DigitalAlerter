...
config['RaspberryPiWebsocket'] = {
    'device': json.dumps([
        {'number': 1, 'url': 'ws://localhost:9453'},
        {'number': 2, 'url': 'ws://localhost:9453'},
        {'number': 3, 'url': 'ws://localhost:9453'},
    ], ensure_ascii=False)
}
...
config['AlertPoint'] = {
    'point': json.dumps([
        {'number': 1, 'name': 'A棟地下前門', 'X': 25,
            'Y': 25, 'cameraLink': [2, 4], 'piLink':1},
        {'number': 2, 'name': 'A棟地下後門', 'X': 50,
            'Y': 50, 'cameraLink': [1, 5], 'piLink':1},
        {'number': 3, 'name': 'B棟東邊前門', 'X': 75,
            'Y': 75, 'cameraLink': [3], 'piLink':2},
        {'number': 4, 'name': 'B棟西邊前門', 'X': 100,
            'Y': 100, 'cameraLink': [1, 3], 'piLink':3},
        {'number': 5, 'name': 'C棟大門', 'X': 125, 'Y': 125,
            'cameraLink': [2, 3, 5], 'piLink':3}
    ], ensure_ascii=False)
}
# 產生攝影機位置資料
config['CameraPoint'] = {
